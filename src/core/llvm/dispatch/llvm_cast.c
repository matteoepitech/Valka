...
llvm_cast(ast_node_t *node, FILE *f, char *dest)
{
   ...
   if (from._id == T_I32 && to._id == T_I64) {
   ...
   if (from._id == T_BOOL && to._id == T_I64) {
       fprintf(f, "%%%s = zext i1 %%%s to i64\n", dest, src);
       return OK_OUTPUT;
   }
   ...
   if (from._id == T_CHAR && to._id == T_I64) {
       fprintf(f, "%%%s = sext i8 %%%s to i64\n", dest, src);
       return OK_OUTPUT;
   }
   ...
   if (from._id == T_FLOAT && to._id == T_I64) {
       fprintf(f, "%%%s = fptosi double %%%s to i64\n", dest, src);
       return OK_OUTPUT;
   }
   if (from._id == T_I64 && to._id == T_I32) {
       fprintf(f, "%%%s = trunc i64 %%%s to i32\n", dest, src);
       return OK_OUTPUT;
   }
   if (from._id == T_I64 && to._id == T_CHAR) {
       fprintf(f, "%%%s = trunc i64 %%%s to i8\n", dest, src);
       return OK_OUTPUT;
   }
   if (from._id == T_I64 && to._id == T_BOOL) {
       fprintf(f, "%%%s = icmp ne i64 %%%s, 0\n", dest, src);
       return OK_OUTPUT;
   }
   if (from._id == T_I64 && to._id == T_FLOAT) {
       fprintf(f, "%%%s = sitofp i64 %%%s to double\n", dest, src);
       return OK_OUTPUT;
   }
