/*
** VALKA PROJECT, 2025
** src/parser/ast/dispatch/ast_while
** File description:
...
/**
 * @brief Fill the content of the while.
 *
 * @param current_token  The current token position
 * @param node           The node of the function
 */
static void
fill_while_content(token_t **current_token, ast_node_t *node)
{
 ...
 */
ast_node_t *
make_ast_while(token_t **current_token, ast_program_t *parent)
{
    ...
    if ((*current_token)->_type != TOKEN_BRACKET_OPEN)
        return NULL;
    move_token(current_token, 1);
    fill_while_content(current_token, node);
