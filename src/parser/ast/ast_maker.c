...
dispatch_ast(token_t **current_token, ast_program_t *parent)
{
    ...
    if (type == TOKEN_SYMBOL && (*current_token)->_next->_type == TOKEN_MATH_OPERATOR &&
        (*current_token)->_next->_next->_type == TOKEN_MATH_OPERATOR)
        return make_ast_increment(current_token, parent);
    if (type == TOKEN_SYMBOL && get_next_token_after_call(*current_token)->_type == TOKEN_ASSIGN)
