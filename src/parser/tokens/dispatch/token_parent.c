/*
** VALKA PROJECT, 2025
** src/parser/tokens/dispatch/token_parent
** File description:
** Token parent
*/
 ...
 */
parsing_src_file_t *
parents_token(parsing_src_file_t *p)
{
    ...
    if (start[0] == '(')
        token = create_token(TOKEN_PARENT_OPEN, start, length,
            p->_current_loc);
    if (start[0] == ')')
        token = create_token(TOKEN_PARENT_CLOSE, start, length,
            p->_current_loc);
